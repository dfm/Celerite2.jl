...
# a, U, V, P = get_celerite_matrices(ar, cr, ac, bc, cc, dc, x, diag)
...
# end
# K0 += Diagonal(diag)
# println(maximum(abs.(K - K0)))

# # Check matmul
# Z = randn(50, 2)
# Y0 = K * Z
# Y = matmul(a, U, V, P, Z)
# println(maximum(abs.(Y - Y0)))

# # Check factor
# factor!(U, P, a, V)
# L0 = cholesky(K).L
# L = LowerTriangular(to_dense(ones(N), U, V, P))
# println(maximum(abs.(L * Diagonal(a) * transpose(L) - K)))
# println(maximum(abs.(L * Diagonal(sqrt.(a)) - L0)))

# # Check dot_tril
# Z = randn(50, 2)
# Y0 = L0 * Z
# dot_tril!(U, P, a, V, Z)
# println(maximum(abs.(Z - Y0)))
